 ...
 ******************************************************************************/
#include "SoftwareI2C.h" //softwire1 channel support for 2 sensors
SoftwareI2C wire1;//wire1 channel1
SoftwareI2C wire2;//wire1 channel1
//SoftwareI2C wire1;//wire1 channel1
//Softwire1 si2c2channel(A6, A7);
//softwarei2c.begin();// sda, scl //Start I2C library
//#include <wire1.h>
#include "SparkFun_VL6180X.h"

VL6180x1::VL6180x1(uint8_t address,int Sda, int Scl)
// Initialize the Library
{
    _i2caddress = address; //set default address for communication
 //wire1.begin(); // Arduino wire1 library initializer
wire1.begin(Sda, Scl);     // wire1, sda, scl
}


uint8_t VL6180x1::VL6180xInit(void){
...
void VL6180x1::VL6180xDefautSettings(void){
  ...
  VL6180x_setRegister(VL6180X_READOUT_AVERAGING_SAMPLE_PERIOD, 0x255); //Set Avg sample period norm set to 0x30
...
void VL6180x1::getIdentification(struct VL6180xIdentification *temp){
...
uint8_t VL6180x1::changeAddress(uint8_t old_address, uint8_t new_address){
...
float VL6180x1::getAmbientLight(vl6180x_als_gain VL6180X_ALS_GAIN)
{
...
uint8_t VL6180x1::VL6180x_getRegister(uint16_t registerAddr)
{
  uint8_t data;

 wire1.beginTransmission( _i2caddress ); // Address set on class instantiation
 wire1.write((registerAddr >> 8) & 0xFF); //MSB of register address
 wire1.write(registerAddr & 0xFF); //LSB of register address
 wire1.endTransmission(); //Send address and register address bytes
 wire1.requestFrom( _i2caddress , 1);
  data =wire1.read(); //Read Data from selected register

  return data;
}

uint16_t VL6180x1::VL6180x_getRegister16bit(uint16_t registerAddr)
{
  uint8_t data_low;
  uint8_t data_high;
  uint16_t data;
 ...
 wire1.requestFrom( _i2caddress, 2);
  data_high =wire1.read(); //Read Data from selected register
  data_low =wire1.read(); //Read Data from selected register
  data = (data_high << 8)|data_low;

  return data;
}

void VL6180x1::VL6180x_setRegister(uint16_t registerAddr, uint8_t data)
{
 wire1.beginTransmission( _i2caddress ); // Address set on class instantiation
 wire1.write((registerAddr >> 8) & 0xFF); //MSB of register address
 wire1.write(registerAddr & 0xFF); //LSB of register address
 wire1.write(data); // Data/setting to be sent to device.
 wire1.endTransmission(); //Send address and register address bytes
}

void VL6180x1::VL6180x_setRegister16bit(uint16_t registerAddr, uint16_t data)
{
 wire1.beginTransmission( _i2caddress ); // Address set on class instantiation
 wire1.write((registerAddr >> 8) & 0xFF); //MSB of register address
 wire1.write(registerAddr & 0xFF); //LSB of register address
  uint8_t temp;
  temp = (data >> 8) & 0xff;
 wire1.write(temp); // Data/setting to be sent to device
  temp = data & 0xff;
 wire1.write(temp); // Data/setting to be sent to device
 wire1.endTransmission(); //Send address and register address bytes
}
...
VL6180x2::VL6180x2(uint8_t address,int Sda, int Scl)
// Initialize the Library
{
    _i2caddress = address; //set default address for communication
 //wire1.begin(); // Arduino wire1 library initializer
wire2.begin(Sda,  Scl);     // wire2, sda, scl
}


uint8_t VL6180x2::VL6180xInit(void){
  uint8_t data; //for temp data storage
...
void VL6180x2::VL6180xDefautSettings(void){
  //Recommended settings from datasheet
  //http://www.st.com/st-web-ui/static/active/en/resource/technical/document/application_note/DM00122600.pdf
  ...
  VL6180x_setRegister(VL6180X_SYSTEM_MODE_GPIO1, 0x10); // Set GPIO1 high when sample complete
  VL6180x_setRegister(VL6180X_READOUT_AVERAGING_SAMPLE_PERIOD, 0x255); //Set Avg sample period
  VL6180x_setRegister(VL6180X_SYSALS_ANALOGUE_GAIN, 0x46); // Set the ALS gain
  VL6180x_setRegister(VL6180X_SYSRANGE_VHV_REPEAT_RATE, 0xFF); // Set auto calibration period (Max = 255)/(OFF = 0)
  VL6180x_setRegister(VL6180X_SYSALS_INTEGRATION_PERIOD, 0x63); // Set ALS integration time to 100ms
  VL6180x_setRegister(VL6180X_SYSRANGE_VHV_RECALIBRATE, 0x01); // perform a single temperature calibration
...
void VL6180x2::getIdentification(struct VL6180xIdentification *temp){
...
uint8_t VL6180x2::changeAddress(uint8_t old_address, uint8_t new_address){
...
float VL6180x2::getAmbientLight(vl6180x_als_gain VL6180X_ALS_GAIN)
{
  //First load in Gain we are using, do it everytime incase someone changes it on us.
  //Note: Upper nibble shoudl be set to 0x4 i.e. for ALS gain of 1.0 write 0x46
  VL6180x_setRegister(VL6180X_SYSALS_ANALOGUE_GAIN, (0x40 | VL6180X_ALS_GAIN)); // Set the ALS gain
...
uint8_t VL6180x2::VL6180x_getRegister(uint16_t registerAddr)
{
  uint8_t data;

 wire2.beginTransmission( _i2caddress ); // Address set on class instantiation
 wire2.write((registerAddr >> 8) & 0xFF); //MSB of register address
 wire2.write(registerAddr & 0xFF); //LSB of register address
 wire2.endTransmission(); //Send address and register address bytes
 wire2.requestFrom( _i2caddress , 1);
  data =wire2.read(); //Read Data from selected register

  return data;
}

uint16_t VL6180x2::VL6180x_getRegister16bit(uint16_t registerAddr)
{
  uint8_t data_low;
  uint8_t data_high;
  uint16_t data;
 ...
 wire2.requestFrom( _i2caddress, 2);
  data_high =wire2.read(); //Read Data from selected register
  data_low =wire2.read(); //Read Data from selected register
  data = (data_high << 8)|data_low;

  return data;
}

void VL6180x2::VL6180x_setRegister(uint16_t registerAddr, uint8_t data)
{
 wire2.beginTransmission( _i2caddress ); // Address set on class instantiation
 wire2.write((registerAddr >> 8) & 0xFF); //MSB of register address
 wire2.write(registerAddr & 0xFF); //LSB of register address
 wire2.write(data); // Data/setting to be sent to device.
 wire2.endTransmission(); //Send address and register address bytes
}

void VL6180x2::VL6180x_setRegister16bit(uint16_t registerAddr, uint16_t data)
{
 wire2.beginTransmission( _i2caddress ); // Address set on class instantiation
 wire2.write((registerAddr >> 8) & 0xFF); //MSB of register address
 wire2.write(registerAddr & 0xFF); //LSB of register address
  uint8_t temp;
  temp = (data >> 8) & 0xff;
 wire2.write(temp); // Data/setting to be sent to device
  temp = data & 0xff;
 wire2.write(temp); // Data/setting to be sent to device
 wire2.endTransmission(); //Send address and register address bytes
}
